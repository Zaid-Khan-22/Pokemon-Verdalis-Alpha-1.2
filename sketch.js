...
rect(405,270,230,5)
  ...
  if(keyDown("space")&&currentBattle.keyCooldown === 0){
    let eff = calculateTypeEffectiveness(
        currentBattle.enemyCurrentPokemonObject.types,
      currentBattle.playerCurrentPokemonObject.moves[currentBattle.moveIndex].type)
    if(currentBattle.enemyPokemonLeft >0){
      if(currentBattle.crit === 1&& eff === 1){
    currentBattle.state = "menu"
    multiplier = 1
      }
      else{
  ...
  if(currentBattle.type === "trainer"){
    ...
    if(currentBattle.turnStart === "player"){
      battleText1 = `${currentBattle.playerCurrentPokemon} used ${currentBattle.pCurrentMove}!`
      if(currentBattle.enemyTeam[0].currentHP<=0){
        battleText2 = `The wild ${currentBattle.enemyCurrentPokemon} fainted!`

      }
      else{
        battleText2 = `The wild ${currentBattle.enemyCurrentPokemon} used ${currentBattle.eCurrentMove}!`
        if(currentBattle.playerTeam[0].currentHP<=0){
          battleText3 = `${currentBattle.playerCurrentPokemon} fainted!`
        }
        else{
    ...
    if(currentBattle.turnStart === "enemy"){
      ...
      if(currentBattle.playerTeam[0].currentHP<=0){
        ...
        battleText2 = `${currentBattle.playerCurrentPokemon} used ${currentBattle.pCurrentMove}!`
        if(currentBattle.enemyTeam[0].currentHP<=0){
          battleText3 = `The ${currentBattle.enemyCurrentPokemon} fainted!`
...
function enemyAI(h1){
  ...
  for(let i = 0;i<noOfMoves;i++){
    ...
    h1.enemyCurrentPokemonObject.moves[i])
...
if(menuPage === "bag"){
  ...
  if(!menuCreated&&keyWentDown("enter")&&move === true){
    menuCreated = true
    move = false
  }
...
function grassHandler(){
  ...
  if(playerLocation === "route_1"&&bgState === 1&&!gc){
